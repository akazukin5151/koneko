...
def test_all_satisfy():
    assert pure.all_satisfy(range(3), lambda x: x == 3) is False
    assert pure.all_satisfy(range(3), lambda x: x < 10) is True
    assert pure.all_satisfy(range(3), lambda x: x >= 0) is True
